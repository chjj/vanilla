...
var router = (function() {
  ...
  return function router(app) {
    ...
    app.route = function() { 
      ...
      var method, path, args = _s.call(arguments);
...
var __stack = exports.stack = [
  ...
  function parse(req, res, next) {
    ...
    // future support for parted, my multipart parser
    if (typeof parted !== 'undefined' 
      && req.type === 'multipart/form-data') {
        // needs fixing
        return parted(req, function(parts) {
          if (parts) {
            req.body = parts;
            next();
          }
...
var sessions = function(opt) {
  ...
  return function(req, res) {
    ...
    if (!id) {
      // dont use crypto here due to talk of it being removed from core
      // cheap way to get decent randomness, seed with the ip address
      id = _s.call(res.socket.remoteAddress + secret).sort(function() {
