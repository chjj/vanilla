...
Response.prototype.sendfile = function(file, next) {
  ...
  fs.stat(file, function on(err, stat) {
    // maybe remove this and just have
    // the static middleware ignore ENOENT
    if (err && err.code === 'ENOENT') {
      return next(); 
    }

    if (err || !stat) {
      return next(err || new Error('No stat.'));
    }
    ...
    if (!stat.isFile()) {
      ...
      return next(err || new Error('Not a file.'));
    }
