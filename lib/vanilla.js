// vanilla - modeled after connect/express/stack/creationix
...
Application.prototype.init = function(func) {
  ...
  this.on('listening', function() {
    ...
    if (!this.get) this.use(this.router);
...
var handler = function(app) {
  ...
  return function(req, res, out) {
    var i = 0; // maybe just do parsePath here
    (function next(err) {
      var func = stack[i++];
      if (!func) {
        if (out) return out(err); // maybe reset app here
        res.setHeader('Content-Type', 'text/plain; charset=utf-8');
        if (err) {
          res.statusCode = 500;
          res.end(app.settings.env === 'development' 
            ? (err.stack || err + '')
...
// maybe users should set the headers themselves
...
vanilla.bodyParser = function(opt) {
  ...
  var multi = vanilla.multipart && vanilla.multipart(opt);
  ...
  return function(req, res, next) {
    ...
    res.end = function() {
      ...
      console.log('Response Time: %s ms for %s %s',
        (Date.now() - start), req.method, req.pathname
      );
      return ret;
    };
    next();
